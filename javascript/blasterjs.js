        ...
        module.exports = blasterjs = Class(
            /** @lends Biojs.blasterjs# */
            {
                constructor: function (options) {
                    ...
                    if (self.opt.string) {
                        var myBlob = new Blob([self.opt.string], {
                            type: "text/plain"
                        });
                        var fakeEvent = {
                            target: {
                ...
                _displayAlignments: function (evt, self) {
                    ...
                    function getHSPWithoutOverlapping(hsps) {
                        ...
                        for (var i = 0; i < hsps.length; i++) {
                            if (parseInt(hsps[i].queryStart) > parseInt(hsps[i].queryEnd)) {
                                hspNoOver.push({
                                    start: parseInt(hsps[i].queryEnd),
                                    end: parseInt(hsps[i].queryStart)
                                });
                            } else {
                                hspNoOver.push({
                                    start: parseInt(hsps[i].queryStart),
                                    end: parseInt(hsps[i].queryEnd)
                                });
                            }
                        }
                    ...
                    function selectAlignment(alignment) {
                        ...
                        document.getElementById(alignment).style.backgroundColor = "powderblue";
                    }
                    ...
                    function createSingleAlignment(alignment, self, hsps, hsp) {
                        ...
                        alignmentDiv.style.padding = "15px";
                        ...
                        buttonDownloadPNG.onclick = function () {
                            downloadSingleAlignmentImg(alignment, 'PNG');
                        };
                        buttonDownloadJPEG.className = 'btn';
                        buttonDownloadJPEG.innerHTML = 'Download as JPEG';
                        buttonDownloadJPEG.onclick = function () {
                            downloadSingleAlignmentImg(alignment, 'JPEG');
                        };
